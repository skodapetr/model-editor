...
import { binarySearch } from "../../utilities/functions";
...
export const useAlignmentController = (props: {
    ...
    const alignmentNodesChange = (changes: NodeChange<NodeType>[]) => {
        ...
        const changedAlignmentLines: Coordinate[] = [];     // TODO: We just info that alignment node changed, so we just need booleans
        ...
        if(changedAlignmentLines.length === 0 && (indexInSortedArray.x >= 0 || indexInSortedArray.y >= 0)) {
            ...
            // internalNode.internals.positionAbsolute.x = alignedInternalNodeCopy.internals.positionAbsolute!.x;
            // internalNode.internals.positionAbsolute.y = alignedInternalNodeCopy.internals.positionAbsolute!.y;
            return;
        }
    ...
    const setNewAlignmentLine = (coordinate: Coordinate, coordinatePosition: number): void => {
        ...
        setAlignmentHelperLine(point);          // TODO: Could also have the end-point as in the old version
    };
    ...
    const checkForAlignment = (absolutePosition: XYPosition, coordinate: Coordinate, changedAlignmentLines: Coordinate[]): [boolean, number] => {
        ...
        if(indexInSortedArray >= 0 && (alignmentNode === null || absolutePosition[coordinate] !== alignmentNode.internals.positionAbsolute[coordinate])) {
            setAlignmentNode(reactFlowInstance.getInternalNode(sortedNodes[indexInSortedArray]?.[0] as string) ?? null);
